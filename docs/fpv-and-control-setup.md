...
2. Set camera mass, vertical field of view and upward tilt. Expand **Installation position & dimensions** for X/Y/Z, housing size and mount roll/pitch/yaw. Negative Z moves the camera upward; positive X moves it forward. Inspect the mounting position in the model.
3. Choose **Apply & fly**, then **FPV**. **V**, or the assigned camera shortcut, cycles Pilot → Chase → FPV. Aircraft without a camera cycle only Pilot/Chase. **F** returns to the pilot and locates the aircraft.
...
Moving or replacing the camera changes total mass, three-axis CG and inertia. Removing it removes that mass component. The camera housing follows the same mount orientation as its optical center. FPV is a rigid view: banking tilts the horizon, a steep camera angle points upward when parked, and badly placed airframe parts can obstruct the lens. Pilot position, physics and render budgets are unchanged by switching views. There is one render pass, not a second camera feed.
