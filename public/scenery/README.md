...
- The bundled CC0 HDR sky is decoded once and reduced at runtime to a shared 1024 × 512 log-RGB RGBA8 texture (about 2.7 MiB with mipmaps). It restores photographic clouds with field-specific haze and aligned sunlight, without an environment convolution or atmospheric scattering pass. The 5.2 MB source is cached by the browser; decoding has a temporary CPU memory cost. A gradient remains the loading/error fallback. Aircraft detail, directional lighting and 1024² contact shadows remain.
...
The foliage cutout threshold rejects the atlas background fringe. Flat-field canopy shadows use one extra instanced card per plant and share the atlas. Counts and the aircraft shadow-map budget are unchanged.
