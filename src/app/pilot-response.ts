...
export class ResponsePanel {
  ...
  render() {
    ...
    for (const prefix of ["flight", "editor"]) {
      ...
      <p class="small muted">${quad ? `Softer ${this.aircraft?.multirotor?.mode === "angle" ? "tilt" : "rotation rate"} commands. Throttle stays manual.` : "Lower rates soften stick response. They do not self-level the aircraft."}</p>
