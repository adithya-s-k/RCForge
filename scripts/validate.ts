...
try {
  ...
  for (const id of ids) {
    ...
    console.log(
      `  ${a.vehicleType === "multirotor" ? "Hover" : `${a.vtol ? "VTOL hover" : `${a.reference.trimSpeedMps ?? 12} m/s`} trim`}: ${trim.converged ? "converged" : "NOT CONVERGED"} · pitch ${trim.pitchDeg.toFixed(2)}° · elevator ${trim.controls.pitch.toFixed(3)} · throttle ${trim.controls.throttle.toFixed(3)}`,
    );
    if (!trim.converged)
      console.warn(
